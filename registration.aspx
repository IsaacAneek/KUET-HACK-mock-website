<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="registration.aspx.cs" Inherits="kuet_hack_mock.registration" %>

<!DOCTYPE html><html xmlns="http://www.w3.org/1999/xhtml"><head runat="server">
    <title></title>
    <link rel="stylesheet" href="styles/login.css" /></head><body>
    <div class="login-nav-bar">
        <img src="assets/hack_logo.jpg" alt="Alternate Text" />
        <p>Hardware Acceleration Club KUET (HACK)</p>
    </div>
    <div class="main-login-section">
        <p class="register-label">Register</p>

        <form id="form1" runat="server" class="form-section">
            <asp:Label Text="Username" runat="server" />
            <asp:TextBox ID="username" runat="server" />

            <asp:Label Text="Password" runat="server" />
            <asp:TextBox ID="password" runat="server" TextMode="Password" />

            <asp:Label Text="Roll" runat="server" />
            <asp:TextBox ID="roll" runat="server" />

            <asp:Label Text="Department" runat="server" />
            <asp:TextBox ID="department" runat="server" />

            <asp:Label Text="Email" runat="server" />
            <asp:TextBox ID="email" runat="server" TextMode="Email" />
            <asp:Button CssClass="register-button2" Text="Register" OnClick="onRegisterClicked" runat="server" />
        </form>
    </div></body></html>